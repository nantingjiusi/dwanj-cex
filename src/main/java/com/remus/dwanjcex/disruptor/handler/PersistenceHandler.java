...
public class PersistenceHandler implements EventHandler<DisruptorEvent> {
    ...
    // 【关键修复】使用 @Lazy 打破循环依赖
    public PersistenceHandler(WalletService walletService, OrderMapper orderMapper, TradeMapper tradeMapper, SymbolService symbolService, ApplicationEventPublisher eventPublisher, @Lazy PersistenceHandler self) {
    ...
    private void handlePlaceOrderPersistence(DisruptorEvent event) {
        if (event.getCancelledOrderIds() != null && !event.getCancelledOrderIds().isEmpty()) {
            for (Long cancelledOrderId : event.getCancelledOrderIds()) {
                try {
                    self.processCancelOrder(cancelledOrderId, "Cancelled by market order self-trade");
                } catch (Exception e) {
        ...
        if (trades != null && !trades.isEmpty()) {
            ...
            try {
                self.updateMarketOrderAvgPrice(event.getOrderId(), totalTradedQty, totalTradedCost);
            } catch (Exception e) {
        ...
        try {
            self.processFinalOrderState(event.getOrderId(), event.isSelfTradeCancel());
        } catch (Exception e) {
    ...
    private void handleCancelOrderPersistence(DisruptorEvent event) {
        ...
        try {
            self.processCancelOrder(dto.getOrderId(), "Cancelled by user");
        } catch (Exception e) {
    ...
    public void updateMarketOrderAvgPrice(Long orderId, BigDecimal totalTradedQty, BigDecimal totalTradedCost) {
        OrderEntity incomingOrder = orderMapper.selectById(orderId);
    ...
    public void processFinalOrderState(Long orderId, boolean isSelfTradeCancel) {
        OrderEntity finalOrderState = orderMapper.selectById(orderId);
