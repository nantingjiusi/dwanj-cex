...
public class MarketMakerBotETH {
    ...
    private static final String SYMBOL = "ETHUSDT"; // 【修改】交易对
    private static final BigDecimal INITIAL_USDT = new BigDecimal("1000000");
    private static final BigDecimal INITIAL_ETH = new BigDecimal("1000"); // 【修改】初始ETH
    private static final BigDecimal BASE_PRICE = new BigDecimal("3000"); // 【修改】基准价格

    public static void main(String[] args) throws InterruptedException {
        System.out.println("--- ETH Market Maker Bot Starting ---");
        List<BotUser> bots = new ArrayList<>();

        // 1. 初始化用户
        for (int i = 0; i < USER_COUNT; i++) {
            String username = "bot_eth_" + System.currentTimeMillis() + "_" + i;
            String password = "123";
            try {
                // 注册
                ...
                deposit(token, "ETH", INITIAL_ETH); // 【修改】充值ETH

                bots.add(new BotUser(username, token));
                System.out.println("Initialized ETH bot: " + username);
            } catch (Exception e) {
                System.err.println("Failed to initialize ETH bot " + username + ": " + e.getMessage());
            }
        }

        System.out.println("\n--- All ETH bots initialized. Starting concurrent trading... ---");
            ...
            executor.submit(() -> {
                while (true) {
                    try {
                        ...
                        System.err.println("An error occurred while placing ETH order for " + bot.username + ": " + e.getMessage());
                        try {
                            Thread.sleep(1000);
    ...
    private static OrderDto createRandomOrder() {
        ...
        if (type == OrderTypes.OrderType.LIMIT) {
            price = currentPrice;
            // 【修改】ETH数量范围 0.1 ~ 1.1
            amount = BigDecimal.valueOf(random.nextDouble() + 0.1).setScale(6, RoundingMode.HALF_UP);
        } else {
            if (side == OrderTypes.Side.BUY) {
                quoteAmount = currentPrice.multiply(BigDecimal.valueOf(random.nextDouble() + 0.1)).setScale(2, RoundingMode.HALF_UP);
            } else {
                amount = BigDecimal.valueOf(random.nextDouble() + 0.1).setScale(6, RoundingMode.HALF_UP);
            }
        }
